...
calBody = fopen('pa2-debug-a-calbody.txt');
...
calReadings = fopen('pa2-debug-a-calreadings.txt');
...
C_est = zeros(numEmMarkers*numFrames,3);
for i=1:numFrames
    ...
    for j=1:numEmMarkers
        ...
        C_est(index+j,:) = invFd_R*aTrans(j,:)' + invFd_p;
    end
